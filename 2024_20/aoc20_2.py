...
def count_cheats(grid, S, E, w, h, max_cheat_len=20, save_threshold=100):
    distS = bfs_on_track(grid, S, w, h)
    distE = bfs_on_track(grid, E, w, h)
    if E not in distS:
        raise ValueError("Финиш недостижим по треку.")
...
def main():
    ...
    ans = count_cheats(grid, S, E, w, h, max_cheat_len=20, save_threshold=100)
    print(ans)
